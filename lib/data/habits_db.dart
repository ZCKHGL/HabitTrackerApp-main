...
class HabitsDb {
  ...
  Future<List<Habit>> loadHabits() async {
    if (_fallbackMemory) {
      final list = _memHabits.values.map(Habit.fromMap).toList()..sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase()));
      for (final h in list) {
  ...
  Future<void> insertHabit(Habit h) async {
    ...
    final db = await _open();
    await db.insert('habits', h.toMap(), conflictAlgorithm: ConflictAlgorithm.abort);
  }
  ...
  Future<void> incrementCompletion(String habitId, String day, int delta) async {
    if (_fallbackMemory) {
      final m = _memCompletions.putIfAbsent(habitId, () => {});
